...
init_temp = sense.get_temperature()
init_humidity = sense.get_humidity()
...
def start_face_reg():
    ...
    faceCascade = cv2.CascadeClassifier("/home/pi/Desktop/437taus23/labs/lab1/two/haarcascade_frontalface_default.xml")
...
while 1:
    ...
    print(f"The current temperature and humidity is: {temp} and {humidity}")
    time.sleep(1) #just so we don't print out so much or check so much

    if ( temp > init_temp + 1 or humidity > init_humidity + 1 ):
